...
\begin{tikzpicture}[
  font=\scriptsize,
  raw/.style={draw, rounded corners, fill=boxcol, align=center, inner sep=3pt, minimum height=7mm},
  op/.style={draw, rounded corners, fill=stagecol!18, align=center, inner sep=3pt},
  good/.style={draw, rounded corners, fill=goodbg, align=center, inner sep=2.5pt},
  bad/.style={draw, rounded corners, fill=badbg, align=center, inner sep=2.5pt},
  ar/.style={-{Latex[length=1.6mm]}, thick, gray!70},
]
\node[raw] (raw) {Raw math\\(English)};
% naive lane
\node[op, above right=3mm and 7mm of raw] (naive) {\textbf{Naive refine}\\$+\,$\texttt{\char92 boxed} $+$ En rules};
\node[bad, right=7mm of naive] (n2) {En GSM8K \textbf{81}\,\ding{51}\\Zh CMATH \textbf{35}\,\ding{55}};
\node[text=misdred, align=center, right=2mm of n2, font=\scriptsize\bfseries] (ntax) {format\\tax};
% MISD lane
\node[op, below right=3mm and 7mm of raw, fill=misdgreen!15] (misd) {\textbf{MISD (ours)}\\verify, no scaffold};
\node[good, right=7mm of misd] (m2) {En GSM8K \textbf{69}\\Zh CMATH \textbf{77}\,\ding{51}};
\node[text=misdgreen, align=center, right=2mm of m2, font=\scriptsize\bfseries] (mbal) {$+8.9$\\9-bench};
\draw[ar] (raw) -- (naive); \draw[ar] (naive) -- (n2);
\draw[ar] (raw) -- (misd);  \draw[ar] (misd) -- (m2);
\end{tikzpicture}
\caption{\textbf{The format-scaffolding generalization tax.} Naively rewriting math SFT data into a canonical \texttt{\char92 boxed} English format lifts in-distribution extraction (GSM8K) but collapses cross-lingual transfer (CMATH $-30$). Our framework \textbf{MISD} verifies structure \emph{without} imposing the scaffolding, staying balanced and winning the honest nine-benchmark suite by $+8.9$.}
